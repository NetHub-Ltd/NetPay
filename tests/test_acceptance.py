...
async def test_admin_login_from_env_bootstrap(client: AsyncClient):
    res = await client.post("/auth/login", json={"email": "admin@nethub.test", "password": "ChangeMeAdmin123!"})
    assert res.status_code == 200, res.text
    assert "access_token" in res.json()

@pytest.mark.asyncio
async def test_worker_path_rejects_missing_internal_key(client: AsyncClient):
    res = await client.post("/internal/events", json={
        "event_id": "evt_1", "provider": "mpesa", "event_type": "stk.callback",
        "integration": {"public_id": "gw_test"}, "payload": {},
    })
...
async def test_worker_path_rejects_wrong_internal_key(client: AsyncClient):
    res = await client.post("/internal/events", json={
        "event_id": "evt_2", "provider": "mpesa", "event_type": "stk.callback",
        "integration": {"public_id": "gw_test"}, "payload": {},
...
async def test_worker_path_accepts_valid_key(client: AsyncClient):
    res = await client.post("/internal/events", json={
        "event_id": "evt_3", "provider": "mpesa", "event_type": "stk.callback",
