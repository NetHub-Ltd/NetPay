...
async def probe_webhook_liveness(url: str, secret: str) -> None:
    payload = json.dumps({"event": "nethub.webhook.probe", "ts": datetime.now(timezone.utc).isoformat()})
...
async def fanout_webhooks(session: AsyncSession, tenant_id: UUID, intent_id: UUID, payload: dict[str, Any]) -> None:
    result = await session.exec(select(Webhook).where(Webhook.tenant_id == tenant_id))
    for h in result.all():
